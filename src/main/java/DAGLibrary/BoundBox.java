...
public class BoundBox {
    ...
    protected static BoundBox updateBoundBox(Point point) {
        ...
        for (Point child: ((Origin) point).getChildren_()) {
            ...
            if (child.getClass() != Origin.class) {
                ...
                BoundBox child_origin_bounds = updateBoundBox(child);
                // There was no valid coordinate assigned to the childBox yet.
                if (countedBox == null) {
                    if (child_origin_bounds != null) {
                        child_origin_bounds = child_origin_bounds.shiftBox(child.position_);
                    }
                    countedBox = child_origin_bounds;
                } else {
                    if (child_origin_bounds != null) {
                        child_origin_bounds = child_origin_bounds.shiftBox(child.position_);
                    }
                    countedBox = countedBox.unite(child_origin_bounds);
                }
            }
        }
        return countedBox;
     ...
     */
    protected final BoundBox shiftBox(Coord2D shift) {
